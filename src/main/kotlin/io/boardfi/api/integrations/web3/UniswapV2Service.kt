...
class UniswapV2Service(
    ...
    @Transactional
    fun getV2SwapEventsLogs(log: Log, wethPrice: Double) {
        val targetToken = "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
        val swapV2 = decodeHexData(log.data, log.address)
        if (swapV2.token0 == targetToken || swapV2.token1 == targetToken) {
            createTokenFromSwap(swapV2, wethPrice)
        }
    }

    @Transactional
    fun createTokenFromSwap(swap: SwapV2, wethPrice: Double): Boolean {
        val isNew = false;
        var tokenId = ""
        ...
        var tokenAmmountSwapped = 0.0;
        var tokenPrice = 0.0
        if (swap.token0 == "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2") {
            if (swap.amount0In.toDouble() == 0.0) {
                tokenAmmountSwapped = swap.amount0Out.toDouble() / 10.0.pow(18.0)
            } else {
                tokenAmmountSwapped = swap.amount0In.toDouble() / 10.0.pow(18.0)
            }
            tokenPrice =
                ((swap.reserve0.toDouble() / 10.0.pow(18.0)) / (swap.reserve1.toDouble() / 10.0.pow(decimals))) * wethPrice
        } else {
            if (swap.amount1In.toDouble() == 0.0) {
                tokenAmmountSwapped = swap.amount1Out.toDouble() / 10.0.pow(18.0)
            } else {
                tokenAmmountSwapped = swap.amount1In.toDouble() / 10.0.pow(18.0)
            }
            tokenPrice =
                (swap.reserve1.toDouble() / 10.0.pow(18.0)) / (swap.reserve0.toDouble() / 10.0.pow(decimals)) * wethPrice
        }
    ...
    private fun createAndSaveToken(
        swap: SwapV2, tokenId: String, supply: BigInteger, metadata: AlchemyMetadataResponse
    ): Token {
    ...
    fun decodeHexData(data: String, pool: String): SwapV2 {
        ...
        return SwapV2(
            ...
            amount0Out = values[2],
            amount1Out = values[3],
            reserve0 = reserves.first,
            reserve1 = reserves.second
        )
    }
    ...
    fun getReserves(pair: String): Pair<BigInteger, BigInteger> {
        ...
        val reservesResponse = web3j.ethCall(
            Transaction.createEthCallTransaction(null, pair, reservesFunction), DefaultBlockParameterName.LATEST
        ).send()
